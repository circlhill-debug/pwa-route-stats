(() => {
  ...
  var YEARLY_THRESHOLDS = {
    parcelTitan: { key: "parcels", threshold: 1e4, label: "Parcel Pounder" },
  ...
  function loadForecastBadgeData() {
  ...
  async function saveForecastSnapshot(snapshot, options = {}) {
    try {
      ...
      if (supabaseClient && normalized.user_id) {
        try {
          await supabaseClient.from(FORECAST_SNAPSHOT_TABLE).upsert({
  ...
  function buildTrendForecast(dowRaw) {
    const loader = typeof window !== "undefined" && typeof window.loadForecastBadgeData === "function" ? window.loadForecastBadgeData : loadForecastBadgeData;
  ...
  function flattenTagStrings(history, targetDow) {
    ...
    const aggregateByType = /* @__PURE__ */ new Map();
  ...
  function generateForecastText(tagHistory, targetDow) {
    ...
    if (dominantTag && Math.abs(dominantTag.minutes) >= 20) {
      ...
      return `\u{1F4EC} Tomorrow\u2019s trend: Expect about ${absMin} minutes ${polarity} than usual due to ${cause} change.`;
    }
    ...
    function buildRouteModelHistory(model) {
      ...
      if (diagnosticsBtn) {
        diagnosticsBtn.onclick = () => {
          ...
          (_a5 = diagnosticsCard == null ? void 0 : diagnosticsCard.scrollIntoView) == null ? void 0 : _a5.call(diagnosticsCard, { behavior: "smooth", block: "start" });
        };
      }
      if (routeModelHistoryChart && typeof routeModelHistoryChart.destroy === "function") {
        try {
          routeModelHistoryChart.destroy();
        } catch (_) {
        }
      ...
      const recent = [...model.residuals].filter((item) => item && Number.isFinite(item.residMin) && item.iso).sort((a, b) => String(a.iso).localeCompare(String(b.iso))).slice(-30);
      if (!recent.length) {
        card.style.display = "none";
        return;
      }
      ...
      const medianAbsolute = absoluteResiduals.length % 2 ? absoluteResiduals[(absoluteResiduals.length - 1) / 2] : (absoluteResiduals[absoluteResiduals.length / 2 - 1] + absoluteResiduals[absoluteResiduals.length / 2]) / 2;
      ...
      const biasText = Math.abs(averageBias) < 0.5 ? "Near zero" : `${averageBias > 0 ? "+" : ""}${Math.round(averageBias)}m`;
      const biasNote = Math.abs(averageBias) < 0.5 ? "No consistent direction" : averageBias > 0 ? "Routes tend longer than predicted" : "Routes tend faster than predicted";
      metricsEl.innerHTML = [
        { label: "Recent hit rate", value: `${hitRate}%`, meta: `${hits}/${recent.length} within \xB1${hitMinutes}m` },
        ...
        { label: "Model confidence", value: `${r2Pct}%`, meta: `R\xB2 on ${model.n} worked days` }
      ...
      noteEl.textContent = `Latest result: ${latestLabel} \xB7 Actual ${(latest.routeMin / 60).toFixed(2)}h vs ${(latest.predMin / 60).toFixed(2)}h predicted \xB7 ${latest.residMin >= 0 ? "+" : ""}${Math.round(latest.residMin)}m.`;
      ...
      routeModelHistoryChart = new Chart(chartCanvas, {
        ...
        data: {
          ...
          datasets: [{
            ...
            backgroundColor: recent.map((item) => item.residMin > hitMinutes ? "rgba(239, 68, 68, 0.65)" : item.residMin < -hitMinutes ? "rgba(34, 197, 94, 0.65)" : "rgba(148, 163, 184, 0.55)"),
            borderRadius: 3,
            borderSkipped: false
          }]
        },
          ...
          plugins: {
            ...
            tooltip: {
              callbacks: {
                title: (items) => {
                  var _a5;
                  const item = recent[(_a5 = items == null ? void 0 : items[0]) == null ? void 0 : _a5.dataIndex];
                  return (item == null ? void 0 : item.iso) || "Route result";
                },
                label: (ctx) => {
                  const item = recent[ctx.dataIndex];
                  if (!item) return "No result";
                  return [
                    `Actual ${(item.routeMin / 60).toFixed(2)}h \xB7 Predicted ${(item.predMin / 60).toFixed(2)}h`,
...
Enter a date (yyyy-mm-dd) to reinstate, or leave blank to keep all:`, "");
      ...
      if (tbody) {
        ...
        tbody.onclick = async (event) => {
          ...
          if (dismissBtn) {
            ...
            if (!iso) return;
            const ok = await triggerTagDismissForIso2(rows, iso, { residuals, model });
            if (!ok) return;
            return;
          }
    ...
    return {
      ...
      triggerTagDismissForIso: triggerTagDismissForIso2,
  ...
  function createCharts({
    ...
    function saveTrendRangeKey(nextKey) {
      ...
      try {
        localStorage.setItem(TREND_RANGE_KEY, trendRangeKey);
      } catch (_) {
      }
    }
      ...
      parentIds.forEach((cardId) => {
        ...
        if (!control) {
          ...
          control.addEventListener("click", (event) => {
            ...
            buildCharts2(latestRowsForCharts);
          });
    ...
    function buildInsightStrip2(rows) {
      ...
      try {
        ...
        const average = (arr, fn) => {
          ...
          return values.length ? values.reduce((sum2, val) => sum2 + val, 0) / values.length : null;
        };
  ...
  function computeWindowMetrics(profile, rows, options = {}) {
    ...
    const deltaPer1000Volume = avg(totals.volume) > 0 && Number.isFinite(avgDeltaHoursPerDay) ? avgDeltaHoursPerDay / avg(totals.volume) * 1e3 : null;
    return {
      profile,
      rows: selected,
  ...
  function computeDelta(active, baseline) {
    ...
    const value = a - b;
    const pct = Math.abs(b) > 1e-6 ? value / b * 100 : null;
    return { value, pct };
  }
  ...
  function buildEvalVolumeShape(rows) {
    ...
    const currentByWeek = new Map(currentSeries.map((point) => [point.weekIndex, point]));
    const priorByWeek = new Map(priorSeries.map((point) => [point.weekIndex, point]));
    const currentLastWeek = currentSeries.length ? currentSeries[currentSeries.length - 1].weekIndex : 0;
    ...
    })).filter(({ currentPoint, priorPoint }) => currentPoint && priorPoint);
    if (!matchedWeeks.length) {
      textEl.textContent = "Need matching worked weeks in both evaluations.";
    }
    const currentTotalVolume = matchedWeeks.reduce((sum, { currentPoint }) => sum + (currentPoint.totalVolume || 0), 0);
    const currentWorkedDays = matchedWeeks.reduce((sum, { currentPoint }) => sum + (currentPoint.workedDays || 0), 0);
    const priorTotalVolume = matchedWeeks.reduce((sum, { priorPoint }) => sum + (priorPoint.totalVolume || 0), 0);
    const priorWorkedDays = matchedWeeks.reduce((sum, { priorPoint }) => sum + (priorPoint.workedDays || 0), 0);
    ...
    if (roundedPct == null) {
      ...
      textEl.textContent = `Current eval is tracking near prior eval cumulatively through week ${compareThroughWeek}.`;
    } else {
      textEl.textContent = `Current eval is ${roundedPct > 0 ? "heavier" : "lighter"} by ${Math.abs(roundedPct)}% cumulatively through week ${compareThroughWeek}.`;
    }
    card.style.display = "";
        ...
        datasets: [
          {
            ...
            data: priorPoints.map((point) => point ? +point.avgVolume.toFixed(1) : null),
          ...
          },
          {
            ...
            data: currentPoints.map((point) => point ? +point.avgVolume.toFixed(1) : null),
            borderColor: "#7ab8ff",
  ...
  function renderEvalTrendList(container, profiles, rows, options = {}) {
    if (!container) return;
    const ordered = [...profiles || []].sort((a, b) => getEvalProfileSortValue(a) - getEvalProfileSortValue(b)).slice(-6);
    if (!ordered.length) {
  ...
  var evalTrendList = document.getElementById("evalTrendList");
  ...
  var sleepWeekBadge = document.getElementById("sleepWeekBadge");
  var sleepWeekMeta = document.getElementById("sleepWeekMeta");
  var sleepRecentBadge = document.getElementById("sleepRecentBadge");
  ...
  function parseMisdeliveryFromWeatherString(weatherStr) {
    if (!weatherStr) return 0;
    try {
      const part = String(weatherStr).split("\xB7").map((s) => s.trim()).find((p) => /^Misdelivery:/i.test(p));
      if (!part) return 0;
      const raw = part.split(":").slice(1).join(":").trim();
      const val = parseFloat(raw);
      if (!Number.isFinite(val) || val <= 0) return 0;
  ...
  function renderYearlyBadges() {
    ...
    try {
      ...
      const records = recordDefs.map((def) => ({ def, best: computeRecord(def) })).filter((entry) => entry.best);
...
Entries are filtered by this id.`);
  ...
  function buildSnapshot(rows) {
    ...
    if (routeExpectedEl && routeExpectedMeta && routeHitMissEl && routeHitMissMeta) {
      ...
      routeExpectedMeta.textContent = hasModel && Number.isFinite(routeModelPredictedMinutes) ? `Route model \xB7 R\xB2 ${(Math.max(0, Math.min(1, model.r2 || 0)) * 100).toFixed(0)}%` : Number.isFinite(predictedRouteMin) ? "Weekday avg fallback" : "Route model pending";
      if (!Number.isFinite(actualRouteMin) || routeResidualMin == null) {
        ...
        routeHitMissMeta.innerHTML = `<span class="${deltaClass}">${routeHitMiss || "\u0394"} ${deltaPrefix}${routeResidualMin}m</span> \xB7 Pred ${(predictedRouteMin / 60).toFixed(2)}h`;
      }
...
${lettersSummary}`;
  ...
  function buildEvalCompare(rows) {
    ...
    try {
      ...
      if (evalWindowCompare) {
        evalWindowCompare.innerHTML = priorProfiles.map((profile) => `<option value="${profile.profileId}">${getEvalProfileDisplayName(profile)}</option>`).join("");
      ...
      if (evalPrimaryMeta) {
        ...
        const progressText = progress.hasActiveWindow ? `Evaluation progress: ${formatNumber(progress.progressPct, 1)}% (${progress.elapsedDays}/${progress.totalDays} days, ${progress.remainingDays} remaining)` : "Evaluation progress: No active eval window";
        const blockText = block ? `2-week check: Block ${block.blockNumber} (${block.blockStart.toFormat("LLL d")} - ${block.blockEnd.toFormat("LLL d")}), ${block.daysRemaining} day(s) left${block.nextCheck ? ` \u2022 Next check around ${block.nextCheck.toFormat("LLL d")}` : ""}` : "2-week check: No active eval window";
  ...
  function buildSleepDrinkChart(rows) {
    ...
    const weekSleepValues = list.filter((r) => {
      if (r.sleep == null || !Number.isFinite(r.sleep)) return false;
      const d = DateTime.fromISO(r.work_date, { zone: ZONE });
      return d.isValid && d >= start2 && d <= end2;
    }).map((r) => Number(r.sleep));
    const weekSleepAvg = weekSleepValues.length ? weekSleepValues.reduce((sum, v) => sum + v, 0) / weekSleepValues.length : null;
    const sleepLogged = list.filter((r) => r.sleep != null && Number.isFinite(r.sleep)).sort((a, b) => String(a.work_date).localeCompare(String(b.work_date)));
    const recentSleep = sleepLogged.slice(-14).map((r) => Number(r.sleep));
    const recentSleepAvg = recentSleep.length ? recentSleep.reduce((sum, v) => sum + v, 0) / recentSleep.length : null;
    ...
    if (sleepDrinkNote) {
      ...
      sleepDrinkNote.textContent = `${drinkText} \xB7 ${sleepText}`;
    }
    drinkWeekBadge.title = avg != null ? `Avg = ${avg.toFixed(1)} days per week` : "";
    if (typeof Chart === "undefined") {
      if (sleepDrinkNote) {
        sleepDrinkNote.textContent += " \xB7 Chart.js missing";
      }
      return;
    }
    if (sleepTrendChart && typeof sleepTrendChart.destroy === "function") {
    ...
    }).filter((r) => r.date.isValid && r.date >= cutoff).sort((a, b) => a.date.toMillis() - b.date.toMillis());
    const trendLabels = trendRows.map((r) => r.date.toFormat("LLL d"));
    const trendValues = trendRows.map((r) => r.sleep);
    const targetValues = trendRows.map(() => 7);
    if (sleepTrendCanvas) {
