...
function sanitizeEvalProfile(input, fallback) {
  ...
  const evalCode = typeof merged.evalCode === "string" && merged.evalCode.trim() ? merged.evalCode.trim() : base.evalCode;
...
export function ensureWeeklyBaselines(rows) {
  try {
    ...
    const snap = { weekStart: weekStartIso, parcels: parcels, letters: letters };
