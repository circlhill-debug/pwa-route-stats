...
export function createCharts({
  ...
  function buildMixViz(rows){
    ...
    const routeThisBy = aggregateByDow(W0, r=> Math.max(0, (+r.route_minutes||0) - boxholderAdjMinutes(r)));
    const routeLastBy = aggregateByDow(W1full, r=> Math.max(0, (+r.route_minutes||0) - boxholderAdjMinutes(r)));
    const routeLastSameBy = aggregateByDow(W1, r=> Math.max(0, (+r.route_minutes||0) - boxholderAdjMinutes(r)));
