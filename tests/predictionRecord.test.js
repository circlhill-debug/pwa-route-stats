...
describe('buildPredictionRecord', () => {
  it('builds a weekday-average prediction and compares it to today actuals', () => {
    const rows = [
      ...
      { work_date: '2026-04-17', status: 'worked', start_time: '08:30', hours: 7.5, office_minutes: 2.1, route_minutes: 5.4, end_time: '15:30' }
    ];
    ...
    expect(record.predicted.endTime).toBe('3:15 PM');
    ...
    expect(record.delta.endMinutes).toBe(15);
  ...
  it('falls back to total-hour delta when clock times are unavailable', () => {
    const rows = [
      { work_date: '2026-04-10', status: 'worked', hours: 7.0, office_minutes: 2.0, route_minutes: 5.0 },
      { work_date: '2026-04-03', status: 'worked', hours: 6.5, office_minutes: 1.75, route_minutes: 4.75 },
      { work_date: '2026-04-17', status: 'worked', hours: 7.5, office_minutes: 2.1, route_minutes: 5.4 }
    ];
    ...
    expect(record.delta.endMinutes).toBeNull();
    expect(record.delta.totalMinutes).toBe(45);
    expect(record.delta.hitMiss).toBe('miss');
  });
});
